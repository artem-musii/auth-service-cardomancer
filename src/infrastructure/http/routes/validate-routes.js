import { extractBearerToken, secureCompare } from '../../../shared/utils.js'
...
const validateRoutes = (app, { sessionService, serviceKey, log }) => {
  app.post('/auth/validate', async ({ body, headers, set }) => {
    ...
    if (!secureCompare(headers['x-service-key'] || '', serviceKey)) { log.warn('validation failed: invalid service key'); set.status = 403; return { error: 'Invalid service key' } }
    const { token } = body
