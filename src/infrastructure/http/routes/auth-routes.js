...
const authRoutes = (
  ...
  app.post(
    ...
    async ({ body, set, server, request }) => {
      ...
      if (existing && !existing.emailVerifiedAt) {
        ...
        try {
          ...
          // intentional: OTP send failure is non-fatal during re-register
        }
      ...
      try {
        ...
        try {
          ...
          // intentional: OTP send failure is non-fatal during registration
        }
    ...
    },
    {
      body: t.Object({
        ...
        displayName: t.Optional(t.String({ pattern: '^[a-z0-9_]{5,32}$' })),
      }),
    },
  )
  ...
  app.post(
    ...
    async ({ body, set, server, request }) => {
      ...
      if (!user.emailVerifiedAt) {
        log.warn('login with unverified email', { email: maskEmail(email) })
        set.status = 403
  ...
  app.post(
    ...
    async ({ body, headers, set }) => {
      ...
      try {
        ...
        if (e.message === 'Invalid display name') {
          set.status = 400
          return { error: e.message }
        }
        if (e.message === 'Display name already taken') {
          set.status = 409
          return { error: e.message }
        }
        set.status = 500
